...
10.times do |index|
  ...
  a = Artwork.create(name: Faker::Music::Opera.verdi, description: Faker::Lorem.sentence(word_count: 15, supplemental: true, random_words_to_add: 16), price: Faker::Number.decimal(l_digits: 2), stock: Faker::Number.between(from: 1, to: 15), creator: Faker::Boolean.boolean(true_ratio: 0.2), category_id: Faker::Number.within(range: 1..10), user_id: Faker::Number.within(range: 1..10) )
  c = Category.create(name: Faker::IndustrySegments.industry)
  cr = Cart.create(user_id: Faker::Number.between(from: 1, to: 10))
  ...
  o = Order.create(user_id: Faker::Number.between(from: 1, to: 10), date: Faker::Date.in_date_period)
